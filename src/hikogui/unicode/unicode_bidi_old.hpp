...
/** Determine run-lengths of embedding-levels.
 * 
 ...
 */
[[nodiscard]] constexpr std::vector<size_t> unicode_bidi_BD7(std::span<int8_t> embedding_levels)
{
    std::vector<size_t> r;
    ...
    for (size_t i = 1; i < embedding_levels.size(); ++i) {
        if (embedding_levels[i] != embedding_level) {
            ...
            r.push_back(i - run_start);
            run_start = i;
        }
    }
    r.push_back(embedding_levels.size() - run_start);
